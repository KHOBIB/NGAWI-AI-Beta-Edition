...
const PERF_LITE_KEY = "ngawi-perf-lite";
...
function detectPerfLiteMode() {
  ...
  return saveData || !!reduceMotion || (dm > 0 && dm <= 4) || (hc > 0 && hc <= 4);
}

const isPerfLite = detectPerfLiteMode();
window.__NGAWI_PERF_LITE__ = isPerfLite;
...
function hydrateLazyImages(rootEl = document) {
...
window.askAI = async () => {
  ...
  try {
    ...
    selectedImages.forEach((imgBase64) => {
      ...
      displayHtml += `${separator}<div class="chat-image-wrap"><img src="${LAZY_IMG_PLACEHOLDER}" data-src="${imgBase64}" class="chat-sent-img" loading="lazy" decoding="async" onclick="window.openImageViewer('${imgBase64}')"></div>`;
    });
...
(function () {
  ...
  function drawVisualizer(now = 0) {
    ...
    let frameInterval = isMobileViewport()
      ? audio.paused
        ? 1000 / 12
        : 1000 / 24
      : audio.paused
        ? 1000 / 18
        : 1000 / 36;
    if (isPerfLite) frameInterval *= 1.8;
